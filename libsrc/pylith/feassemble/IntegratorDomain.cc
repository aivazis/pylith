...
void
pylith::feassemble::IntegratorDomain::setKernelsResidual(const std::vector<InterfaceResidualKernels>& kernels,
                                                         const pylith::topology::Field& solution) {
    PYLITH_METHOD_BEGIN;
    PYLITH_JOURNAL_DEBUG(_labelName<<"="<<_labelValue<<" setKernelsResidual(# kernels="<<kernels.size()<<")");
...
void
pylith::feassemble::IntegratorDomain::setKernelsJacobian(const std::vector<InterfaceJacobianKernels>& kernels,
                                                         const pylith::topology::Field& solution) {
    PYLITH_METHOD_BEGIN;
    PYLITH_JOURNAL_DEBUG(_labelName<<"="<<_labelValue<<" setKernelsLHSJacobian(# kernels="<<kernels.size()<<")");
    ...
    for (size_t i = 0; i < kernels.size(); ++i) {
        ...
        const PetscInt i_fieldBasis = solution.getSubfieldInfo(kernels[i].subfieldBasis.c_str()).index;
        const PetscInt i_part = kernels[i].part;
        err = PetscWeakFormAddBdJacobian(dsLabel.weakForm(), dsLabel.label(), dsLabel.value(), i_fieldTrial, i_fieldBasis,
                                         i_part, kernels[i].j0, kernels[i].j1, kernels[i].j2, kernels[i].j3);
        PYLITH_CHECK_ERROR(err);
